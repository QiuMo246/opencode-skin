...
export default function GalleryPage() {
  ...
  useEffect(() => {
    ...
    api.terminalDetect().then(setWt).catch(() => {});
    api.injectorDetect().then(setInj).catch(() => {});
  }, []);
  ...
  const saveTuiTheme = async (apply: boolean) => {
    ...
    try {
      ...
      flash(apply ? `主题「${name}」已保存并应用，重启 opencode 生效` : `主题「${name}」已保存，可在编辑器中微调`);
      ...
      <div className="gallery-grid">
        <div className="gallery-right">
            ...
            <>
              ...
              {palette.length > 0 && (
                <div className="palette-row">
                  {palette.map((s) => (
                    <div key={s.hex} className="swatch" style={{ background: s.hex }} title={`${s.hex} · ${Math.round(s.share * 100)}%`}>
          ...
          <fieldset className="slot-group out-panel">
            ...
            <div className="row">
              <input placeholder="主题名称" value={themeName} onChange={(e) => setThemeName(e.target.value)} />
              <button disabled={!payload || busy} onClick={() => saveTuiTheme(false)}>保存</button>
              <button className="primary" disabled={!payload || busy} onClick={() => saveTuiTheme(true)}>保存并应用</button>
            </div>
          </fieldset>
          ...
          {wt?.found ? (
            <fieldset className="slot-group out-panel">
              ...
              <div className="row">
                ...
                <input type="range" min={5} max={100} value={Math.round(imageOpacity * 100)} onChange={(e) => setImageOpacity(Number(e.target.value) / 100)} />
              ...
              <div className="row">
                <button className="primary" disabled={!payload || busy} onClick={applyWtBackground}>写入背景</button>
                <button className="danger" disabled={!wt.backupExists} onClick={restoreWt}>还原备份{wt.backupExists ? "" : "（无）"}</button>
              </div>
            </fieldset>
          ) : (
          ...
          <fieldset className="slot-group out-panel">
            ...
            <div className="row">
              <input placeholder="皮肤 id（小写字母/数字/-）" value={skinId} onChange={(e) => setSkinId(e.target.value)} />
            ...
            <div className="row">
              ...
              <input type="range" min={0} max={100} value={Math.round(focusX * 100)} onChange={(e) => setFocusX(Number(e.target.value) / 100)} />
              <label className="fld">焦点 Y {(focusY * 100).toFixed(0)}%</label>
              <input type="range" min={0} max={100} value={Math.round(focusY * 100)} onChange={(e) => setFocusY(Number(e.target.value) / 100)} />
            </div>
            <div className="row">
              <button className="primary" disabled={!payload || busy} onClick={makeSkinPack}>生成皮肤包</button>
              <button disabled={!inj?.found || !inj?.launcher} onClick={inject}>调用注入器</button>
            </div>
          </fieldset>
        </div>
        ...
        <div className="gallery-left">
          ...
          >
            ...
            />
            {stage === "compressing" ? "压缩图片中…" : stage === "extracting" ? "取色计算中…" : "点击或拖拽上传图片"}
          </label>
          <h3>内置图库</h3>
          <div className="wp-grid">
            {wallpapers.map((w) => (
              <button key={w.id} className="wp-item" title={w.title} disabled={busy} onClick={() => selectSource(`/api/images/file/${w.id}`)}>
