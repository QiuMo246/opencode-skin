...
async function main() {
  const tabs = (await (await fetch(`http://127.0.0.1:${PORT}/json`)).json()) as Array<{ type: string; webSocketDebuggerUrl?: string; url: string }>;
  const existing = tabs.find((t) => t.type === "page" && t.url.includes("mock.html"));
  if (existing?.webSocketDebuggerUrl) {
    await evaluateOnTarget(existing.webSocketDebuggerUrl, "try{window.__ocSkinEngine__.restore()}catch(e){}; try{localStorage.clear()}catch(e){}");
  ...
  const tabs2 = (await (await fetch(`http://127.0.0.1:${PORT}/json`)).json()) as Array<{ type: string; webSocketDebuggerUrl?: string; url: string }>;
  ...
  const chk1 = await evaluateOnTarget(ws, "JSON.stringify({style:!!document.getElementById('__oc_studio_style__'),cls:document.documentElement.classList.contains('oc-studio-skin'),ls:!!localStorage.getItem('__oc_skin_cfg_v1'),engine:!!window.__ocSkinEngine__})");
  console.log("state:", chk1);

  await evaluateOnTarget(ws, "location.reload()");
  await sleep(2500);
  const ws2 = await firstPageWs();
  const chk2 = await evaluateOnTarget(ws2, "JSON.stringify({style:!!document.getElementById('__oc_studio_style__'),cls:document.documentElement.classList.contains('oc-studio-skin'),ls:(function(){try{return !!localStorage.getItem('__oc_skin_cfg_v1')}catch(e){return 'ERR'}})(),engine:!!window.__ocSkinEngine__,err:window.__ocSkinErr__||null,ready:document.readyState})");
  console.log("after-reload:", chk2);

  await evaluateOnTarget(ws2, "document.getElementById('__oc_studio_style__').remove()");
  await sleep(600);
  const chk3 = await evaluateOnTarget(ws2, "JSON.stringify({observerRestored:!!document.getElementById('__oc_studio_style__')})");
  console.log("observer:", chk3);

  const rs = await restoreOnTarget(ws2);
  await sleep(300);
  const chk4 = await evaluateOnTarget(ws2, "JSON.stringify({style:!!document.getElementById('__oc_studio_style__'),cls:document.documentElement.classList.contains('oc-studio-skin'),ls:!!localStorage.getItem('__oc_skin_cfg_v1')})");
