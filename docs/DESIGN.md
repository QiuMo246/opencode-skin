...
> 版本：v0.1（设计稿）
> 日期：2026-08-25
> 状态：§1–§9 为设计期记录，已按此实现；§10–§11 为实现期增补
...
- "改配色" → 生成/编辑 opencode 主题 JSON（官方原生支持，不触碰程序本体）✅
...
- **目标端策略（据 2.5/2.6 核实结论调整）**：工具定位为**双目标主题工作台**——
  - TUI 目标：编辑 → 保存 → 一键应用到 `tui.json`，全流程在工具内完成
  - 桌面端目标（双轨）：
...
- **实时校验**：内置 `https://opencode.ai/theme.json` schema（构建时拉取固化到本地），非法值即时标红，避免写出会让 opencode 启动失败的文件
...
> 以下是 v0.1 设计期的结构规划，实现期已扩展（当前结构见 README「项目结构」）。
...
| GitHub 匿名 API 限额（60 次/时）                          | 低   | 限额不足时回退到内置精选列表；可选填 PAT 提升限额                                                                |
| 用户终端非 Windows Terminal                               | 低   | 功能优雅降级为"仅取色"，不影响主流程                                                                             |
| truecolor 未开启导致色彩失真                              | 低   | 文档提供 `COLORTERM=truecolor` 检查指引                                                                          |
| 桌面端自定义主题官方未支持（2.5 节）                      | 中   | CDP 皮肤轨现已真实生效；官方格式仅承诺"生成+导出"，apply 接口预留                                                |
...
> 视觉方向由用户选定：工具界面本身就是它所生产的液态玻璃材质的演示。
